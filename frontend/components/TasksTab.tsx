...
import type { TaskConfig, TaskRun, AgentConfig, EnvironmentConfig } from "@/lib/api"
...
export function TasksTab({ selectedName: initialName }: TasksTabProps) {
  ...
  const loadTasks = async () => {
    try {
      const [taskList, agentList, envList] = await Promise.all([
        api.listTasks(),
        api.listAgents(),
  ...
  const isDirty = (() => {
    ...
    return (
      ...
      (draft.environment_id ?? null) !== (original.environment_id ?? null) ||
      draft.prompt !== original.prompt ||
      draft.session_mode !== original.session_mode ||
      (draft.session_alias ?? null) !== (original.session_alias ?? null) ||
      (draft.target_session_alias ?? null) !== (original.target_session_alias ?? null)
    )
  })()
    ...
    >
      <div className="p-6 max-w-2xl space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <Select
            value={draft.environment_id ?? "__default__"}
            onValueChange={(v) => setDraft((d) => d ? { ...d, environment_id: v === "__default__" ? null : v } : d)}
          >
            <SelectTrigger className="h-8 text-sm">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__default__">Use first available</SelectItem>
              {environments.map((e) => (
                <SelectItem key={e.id} value={e.id}>{e.name} — {e.model}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <p className="text-xs text-muted-foreground">Leave as default to use the first configured environment.</p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <Select
            ...
            onValueChange={(v) => setDraft((d) => d ? { ...d, session_mode: v as TaskConfig["session_mode"] } : d)}
          >
            <SelectTrigger className="h-8 text-sm">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="new">New — fresh session per run</SelectItem>
              <SelectItem value="fixed">Fixed — reuse one persistent session</SelectItem>
            </SelectContent>
          </Select>
          <p className="text-xs text-muted-foreground">
            {draft.session_mode === "new" && "A new session is created for each run."}
            {draft.session_mode === "fixed" && "The same session is reused across all runs (alias auto-derived from task name if unset)."}
          </p>
        </div>

        {/* Session alias (fixed only) */}
        {draft.session_mode === "fixed" && (
          <div className="space-y-1.5">
            <Label className="text-sm font-medium">Session alias</Label>
            <Input
              value={draft.session_alias ?? ""}
              onChange={(e) => setDraft((d) => d ? { ...d, session_alias: e.target.value || null } : d)}
              placeholder={draft.name || "task-name"}
              className="h-8 text-sm font-mono"
            />
            <p className="text-xs text-muted-foreground">Alias for the persistent session. Defaults to task name.</p>
          </div>
        )}

        {/* Target session alias (any mode — enables proactive behavior) */}
        <div className="space-y-1.5">
          <Label className="text-sm font-medium">Send to session alias (optional)</Label>
          <Input
            value={draft.target_session_alias ?? ""}
            onChange={(e) => setDraft((d) => d ? { ...d, target_session_alias: e.target.value || null } : d)}
            placeholder="heartbeat"
            className="h-8 text-sm font-mono"
          />
          <p className="text-xs text-muted-foreground">
            If set, a sticky target session is created with this alias before each run. Use{" "}
            <code className="text-xs">{"{{target_session_alias}}"}</code> in the prompt. The agent can call{" "}
            <code className="text-xs">chat_send()</code> to post to it.
          </p>
        </div>
          ...
          <div className="space-y-2 pt-2 border-t border-border">
            ...
            {runsLoading ? (
              ...
              <div className="space-y-0.5">
                {runs.map((run) => {
                  const s = run.scheduled_at
                  const scheduledDate = new Date(s.endsWith("Z") || s.includes("+") ? s : s + "Z")

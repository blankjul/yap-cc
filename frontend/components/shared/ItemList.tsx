  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      if (e.key === navLeader) {
        e.preventDefault()
        if (active) {
          // Double leader → focus the selected item in the sidebar
          clear()
  ...
  return (
    <div className="py-1">
      {items.map((item) => (
        <div key={item.id}>
        {item.dividerBefore && (
          <div className="mx-3 my-1 border-t-2 border-border" />
        )}
        <div
          key={item.id + "-row"}
          className={cn(
            "group flex items-center gap-2 px-2 mx-1 rounded-md",
            selectedId === item.id ? "bg-primary text-primary-foreground" : "hover:bg-accent/50"
          )}
        >
          <button
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {item.sublabel && (
                <p className={cn("text-xs truncate", selectedId === item.id ? "text-primary-foreground/70" : "text-muted-foreground")}>{item.sublabel}</p>
              )}
            </div>

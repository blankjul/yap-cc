...
import type { AgentConfig, EnvironmentConfig } from "@/lib/api"
...
export function HomeScreen() {
  ...
  const { environmentId: selectedEnvId, setDefault: setSelectedEnvId } = useDefaultEnvironment(environments)
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (!trimmed || loading || !selectedAgentId || !selectedEnvId) return
    setLoading(true)
    try {
      const session = await api.createSession({ agent_id: selectedAgentId, environment_id: selectedEnvId })
  ...
  }, [text, loading, selectedAgentId, selectedEnvId, router])
    ...
    <div className="flex flex-col items-center justify-center h-full px-6">
      <div className="w-full max-w-2xl flex flex-col gap-8">
        <div className="text-center space-y-2">
          <img src="/logo.svg" alt="yapflows" className="w-56 mx-auto dark:invert" />
          ...
          <div className="flex items-center justify-between px-4 pb-4 pt-1">
            ...
            <div className="flex items-center gap-2 ml-3 shrink-0">
              ...
              <Button
                ...
                disabled={!text.trim() || loading || !selectedAgentId || !selectedEnvId}
              >

...
type SectionKey = "/" | "/chats" | "/agents" | "/knowledge" | "/memory" | "/tasks" | "/skills" | "/browser" | "/settings"
...
const NAV_SECTIONS: Array<{
  ...
  { path: "/browser",    icon: Monitor,       label: "Browser",   shortcut: "8" },
]
...
function ShellContent({ children }: { children: React.ReactNode }) {
  ...
  const leaderKeysRef = useRef<string[]>(["\\"])
  const [hotkeysOpen, setHotkeysOpen] = useState(false)
  const [sidebarWide, setSidebarWide] = useState(false)
  useEffect(() => {
    setSidebarWide(localStorage.getItem(STORAGE_KEYS.SIDEBAR_WIDE) === "true")
  }, [])

  const toggleSidebar = () =>
    setSidebarWide((v) => {
      localStorage.setItem(STORAGE_KEYS.SIDEBAR_WIDE, String(!v))
      return !v
    })

  // Check setup status on mount; also load leader keys from settings
  useEffect(() => {
    api.setupStatus()
    ...
    api.getSettings()
      .then((s) => {
        const k1 = s.ui?.leader_key || "\\"
        const k2 = s.ui?.leader_key_2 || ""
        leaderKeysRef.current = [k1, ...(k2 ? [k2] : [])]
      })
      .catch(() => {})
  }, [router])
  ...
  useKeyboardShortcuts(
    ...
    leaderKeysRef,
    () => {
      const main = document.querySelector<HTMLElement>("[data-yapflows-main]")
      if (!main) return
      const focusable = main.querySelector<HTMLElement>("textarea, input, [contenteditable=\"true\"]")
      focusable?.focus()
    }
  )
    ...
    <div className="flex h-screen bg-background overflow-hidden">
      ...
      >
        ...
        <NavIcon
          ...
          shortcut="t"
          wide={sidebarWide}
          leaderKey={leaderKeysRef.current[0]}

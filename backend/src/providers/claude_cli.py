...
def _resolve_claude_bin() -> str:
    ...
    if explicit:
        return explicit
    # Try shutil.which with an expanded PATH that includes common user-local dirs
    extra_paths = [
    ...
    search_path = ":".join(extra_paths) + (":" + env_path if env_path else "")
...
class ClaudeCliProvider(BaseProvider):
    ...
    async def run(
        ...
        env["PATH"] = _extra_paths + ":" + env.get("PATH", "/usr/local/bin:/usr/bin:/bin")
        ...
        proc = await asyncio.create_subprocess_exec(
            *args,
            ...
            env=env,
            limit=8 * 1024 * 1024,  # 8 MB — default 64 KB is too small for large JSON events
        )
